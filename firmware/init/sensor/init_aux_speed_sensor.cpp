...
public:
	WheelSlipBasedOnAuxSpeedSensor()
		: Sensor(SensorType::WheelSlipRatio) {}

	SensorResult get() const final override {
		auto value1 = auxSpeed1.get();
		auto value2 =
				engineConfiguration->useVssAsSecondWheelSpeed ? Sensor::get(SensorType::VehicleSpeed) : auxSpeed2.get();
		if (!value1.Valid || !value2.Valid) {
			return UnexpectedCode::Unknown;
		}
		// todo: remove handling of zero in July of 2024
		float correctedAuxSpeed1Multiplier =
				engineConfiguration->auxSpeed1Multiplier == 0 ? 1 : engineConfiguration->auxSpeed1Multiplier;
