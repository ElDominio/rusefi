...
- **`EFI_CHT_CLT_ESTIMATOR` (`firmware/controllers/sensors/cht_clt_estimator.h`/`.cpp`) defaults differently per chip family, and `chtSensor` has no default thermistor curve anywhere**: `stm32f4ems/efifeatures.h` defaults the flag `FALSE`; only `stm32f7ems`/`stm32h7ems` default it `TRUE`. A board built as F4 (check its `meta-info.env` `PROJECT_CPU`, not just which `ifeq`s appear in `board.mk`) needs an explicit `DDEFS += -DEFI_CHT_CLT_ESTIMATOR=TRUE` in `board.mk` or the estimator compiles to the `#else` stub with no error. This flag is a pure C++ compile guard (not referenced by any `.ini`/`.txt` `@@if_...@@`), so it does *not* fall under the "TS page guard flag must live in `prepend.txt`" rule above. Separately, unlike `clt`/`iat` (whose `ThermistorConf.config` gets a generic NTC curve from `engine_configuration.cpp`'s `setDefaultEngineConfiguration()`), `chtSensor.config` is zero-initialized with no default set anywhere in the codebase — wiring `chtSensor.adcChannel` without also giving `chtSensor.config` real curve values trips `validateThermistorConfig`'s ascending-order check (`tempC_1 >= tempC_2 || ...`) and calls `firmwareError` at boot the first time a board configures that channel.
- **`#include "custom_page.h"` (or anything else pulling in `page_N_generated.h`) from a board's `board_configuration.cpp` breaks the OpenBLT bootloader build if the board sets `USE_OPENBLT=yes`**: `board_configuration.cpp` gets compiled twice — once for the real firmware and once for the bootloader (`-DEFI_BOOTLOADER`, invoked via `compile.sh -b` / `build_gui.py`'s bundle build, *not* exercised by a plain non-`-b` build). `page_6_generated.h`'s array fields need curve-size macros (`LAUNCH_POWER_RAMP_CURVE_SIZE`, `BURST_KNOCK_*_SIZE`, `WOT_ENRICHMENT_SIZE`, ...) that are normally supplied by `engine.h`'s unconditional module-header includes — but the bootloader's translation unit of `board_configuration.cpp` never includes `engine.h` (`pch.h` only pulls in `engine_configuration.h`, and the bootloader never runs engine config), so those macros are simply undefined there, producing "was not declared in this scope" errors deep in the generated header. No existing OpenBLT board includes `custom_page.h` from `board_configuration.cpp` for this reason. If you need `getCustomPage()` (page 5/6 config) inside a board's default-configuration function, wrap both the `#include` and the call site in `#ifndef EFI_BOOTLOADER` (mirroring how `engine.h` itself guards `engine_modules_generated.h`) — the call is dead code under the bootloader anyway. When touching `board_configuration.cpp` on any OpenBLT board, test-build with `compile.sh -b <meta-info.env>` (or `build_gui.py`), not just the plain build — the bootloader half is a distinct translation unit with a much smaller include/define set and can break independently.
