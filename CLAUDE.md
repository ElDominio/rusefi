...
**Boards using the `meta-info.env` pattern** (e.g. `protorico-econoline`, `protorico-grummann` - custom Hellen-module boards with a `compile_firmware.sh` instead of a `compile_<board>.sh`): that wrapper does `cd ../../../.. && bash bin/compile.sh config/boards/<board>/meta-info.env`, but from `firmware/config/boards/<board>/` four `..` lands one level *above* `firmware/` (at the repo root), where there is no `bin/compile.sh` (it lives at `firmware/bin/compile.sh`) - the wrapper as checked in does not actually work. Invoke `firmware/bin/compile.sh` directly instead (it self-locates its own `firmware/` dir via `$(dirname "$0")`, so it works from anywhere): `bash firmware/bin/compile.sh config/boards/<board>/meta-info.env -j12`.

**Bringing up a brand new board directory**: a fresh `connectors/connectors.yaml` with no sibling `connectors/generated_*` files yet will fail the very first build with `No rule to make target 'generated_ts_name_by_pin.cpp'` - the `$(CONFIG_FILES): .config-sentinel` rule in `rusefi_config.mk` that would normally regenerate `connectors/generated_board_pin_names.h`/`generated_outputs.h`/`generated_ts_name_by_pin.cpp` (via `gen_config_board.sh` -> `PinoutLogic.java`) doesn't get triggered correctly on that first pass. Fix once with an explicit `cd firmware && bash gen_config_board.sh ./config/boards/<board> <short-name>`; after that, normal builds keep them in sync automatically. Note `git ls-files` shows these three `connectors/generated_*` files, plus `controllers/generated/{engine_configuration_generated_structures,rusefi_generated}_<board>.h` and `tunerstudio/generated/rusefi_<board>.ini`, already checked into git for every existing board - but per "Do not attempt to commit any generated files" above, don't stage newly (re)generated copies of these yourself; leave that for the human, same as any other generated-file diff produced by a build.
...
The build system does not track compiler-flag changes, so run `make clean` in `unit_tests/` when switching between coverage and non-coverage builds — otherwise stale objects produce a near-zero (or needlessly instrumented) result. The same applies when switching `CC` (e.g. `make CC=clang` then back to default GCC, per the "Compiler Flags" cross-compiler check below): without a `make clean` in between, the link mixes leftover object files from the other compiler with newly-built ones, producing deterministic but bogus failures unrelated to any real code change — observed as callback/function-pointer-equality mismatches in unrelated trigger/cranking/fuel-scheduler tests. If a full-suite run fails in areas you didn't touch right after a `CC=` switch, suspect stale mixed objects before suspecting a regression — `make clean` and rebuild.
...
TCU tests (`unit_tests/tests/test_tcu.cpp`) have the same class of pitfall: every production `GearControllerBase`/`TransmissionControllerBase` subclass (`automaticGearController`, `generic4TransmissionController`, etc.) is a file-scope singleton reused by every test, and `initGearController()` only re-points `engine->gearController` at one and calls `init()` — it does not reset `desiredGear`/`currentGear`/`isShifting`/`m_shiftTime`/etc. A test that changes gear/shift state can leak it into whichever test runs next (order-dependent, e.g. `desiredGear` observed as `GEAR_3` instead of an expected `GEAR_2` purely from a prior test's leftover state). `initGearController()` now calls `EFI_UNIT_TEST`-only `resetForUnitTest()` seams on both classes unconditionally (mirrors the DC/idle hardware pattern above) — no action needed by test authors, but be aware a hand-rolled `TestTransmissionController tc;` (stack-local, bypassing the singleton entirely, as in `shiftCompletesWhenTheTargetGearIsDetected`) sidesteps this reset path since it's a separate instance.
...
- **Gauge-name length limit applies to every `LiveData.yaml`-registered `.txt` struct file, not just `output_channels.txt`**: the field comment's *first line* (everything before a literal `\n` in the comment text, or the whole comment if there's no `\n`) becomes the data-log gauge name and is hard-capped at 34 characters (`DataLogConsumer.getHumanGaugeName`, `MSQ_LENGTH_LIMIT`) — exceeding it throws `IllegalStateException` and aborts `LiveDataProcessor` codegen for *every board*, not just the one you're building (e.g. a too-long comment in `firmware/controllers/tcu/tcu_controller.txt` breaks the same way as one in `output_channels.txt`). Keep the first line short (e.g. `"Oil Life %"`) and put a longer description after a literal `\n` (e.g. `"Oil Life %\nOil Life Monitor: estimated remaining oil life."`), matching the existing `actualLastInjectionRatio`-style two-line comment convention.
- **Comment syntax differs between `.txt` config-definition files and `.ini` files**: `firmware/tunerstudio/*.ini` files (`tunerstudio.template.ini`, `top_level_menu.ini`, `gauge_declarations.ini`) use `;` for standalone line comments. The `.txt` struct-definition files consumed by `configuration_definition`/`LiveData` (`rusefi_config.txt`, `config_page_*.txt`, per-module `*.txt` LiveData files like `tcu_controller.txt`) do **not** support `;` as a standalone comment marker — a line starting with `;` there throws `Cannot parse line` from `ReaderStateImpl`. Use `//` (or the legacy `!`) instead.
- **TS "quick gauge" categories** (the picker grouping shown when a user adds a gauge in TunerStudio, e.g. "Transmission", "Knock") are declared separately from the field/output-channel definition: `firmware/tunerstudio/gauge_declarations.ini` has `gaugeCategory = <Name>` sections, each listing `gaugeId = fieldName, "Display Title", "units", lo, hi, ...` presets. Adding a new LiveData field does **not** automatically make it pickable under a category — it needs both the struct field (in the relevant `.txt`, for `entry =`/`indicator =`/`graphLine =` output-channel wiring) *and* a corresponding line under the right `gaugeCategory` block here.
- **Before gating a new `.ini` field/gauge behind an `@@if_<flag>@@` conditional, grep whether any board ever sets that flag `true`** (`grep -rn "#define <flag> true" firmware/config/boards/`). Several alpha-stage flags (e.g. `show_tcu_gauges`) default `false` in `rusefi_config.txt` and are never overridden `true` by any board — gating new content behind one of these makes it permanently invisible in TunerStudio everywhere, with no error or warning anywhere in the build. When a dialog/gauge category has a mix of gated and ungated sibling entries, match the *majority* convention unless you've confirmed the flag actually has a path to `true`.
- **`rusefi_config.txt` fields sharing one `.txt` file can land in two different generated C++ structs**: fields near `tcuEnabled`/`gearControllerMode`/`totalGearsCount` are accessed via `engineConfiguration->` (`engine_configuration_s`), while fields near `tcu_shiftTime`/`tcu_shiftSpeed12` — declared later in the *same* file — are accessed via `config->` (a second struct nested in `persistent_config_s`, roughly the "configuration" vs "calibration/tune" split described above). There's no syntactic marker in the `.txt` for which struct a new field lands in — check which pointer the *nearest neighboring field* uses (e.g. `grep` the field declared just before/after your insertion point in a `.cpp` that already reads it) rather than assuming `engineConfiguration` and `config` are interchangeable aliases. Using the wrong one compiles fine at the `.ini`/codegen level (TS's field namespace is flat) but fails C++ compilation with a "struct X has no member" error that doesn't obviously point at the real cause.
...
**Exception — some generated headers are shared across boards, not board-suffixed, and can go stale after building a different target.** Most of `firmware/controllers/generated/` is per-board (`*_alphax-s550.h`, `*_f407-discovery.h`, etc.), but the `config_page_*.txt`-derived page headers (e.g. `firmware/controllers/generated/page_5_generated.h`) are a single shared file. `make`'s dependency rule only regenerates it when its `.txt` source changes, not when the *board target* changes — so building firmware for board A, then immediately running unit tests (which target `f407-discovery` by default) or building board B, can silently reuse board A's version of that shared header. Symptom: a `static_assert(sizeof(pageN_s) == <size>)` failure whose expected size doesn't match any board you're currently building for. Fix by explicitly forcing regeneration for the target you actually want: `bash firmware/gen_config_board.sh firmware/config/boards/<board> <short-name>` before rebuilding — this is the one legitimate reason to invoke it directly. The same class of staleness hits `firmware/controllers/lua/generated/value_lookup_generated.cpp`/`.md` and `value_lookup_table_generated.cpp` — also written to a single non-board-suffixed path (by `gen_config_common.sh`'s `-field_lookup_file`), also not a tracked `make` dependency for the unit-test build. Symptom there: `'struct persistent_config_s' has no member named 'boardUse...'` compile errors naming fields that belong to whatever board was last generated (e.g. an AlphaX board's `board_config.txt`), not the board actually being built. Neither rebuilding the `config_definition` shadow jar nor `make clean` fixes this — only the same explicit `gen_config_board.sh` invocation for the target you're actually building (e.g. `f407-discovery` for unit tests) does.

**Exception — per-module LiveData `.txt` files and `gauge_declarations.ini` are not tracked `make` dependencies, so editing them alone does not trigger regeneration.** `firmware/docs_enums.mk`'s `DOCS_ENUMS_INPUTS` list (which gates `gen_live_documentation.sh`, the source of `firmware/live_data_generated/*_generated.h`) carries its own `# TODO: are we missing a ton of .txt file references from LiveData.yaml?!` comment — confirmed true: a per-module LiveData struct file like `firmware/controllers/modules/fuel_pump/fuel_pump_control.txt` is absent from it, so changing a field there (type, scale, comment) does not by itself cause `make` to rerun the LiveData generator; the stale header is silently reused and the build still succeeds. Separately, `firmware/tunerstudio/gauge_declarations.ini` (the TS "quick gauge" category presets) is read by the config-definition tool and its content ends up inlined under `[GaugeConfigurations]` in the generated per-board `.ini`, but it is absent from `firmware/rusefi_config.mk`'s `CONFIG_INPUTS`, so editing it alone doesn't invalidate that generated `.ini` either. In both cases the fix is to touch a file that *is* a tracked dependency to force regeneration — `firmware/integration/LiveData.yaml` for the first case, `firmware/tunerstudio/tunerstudio.template.ini` for the second — then rebuild. After touching and rebuilding, verify by inspecting the actual generated output (the struct field's C++ type in `*_generated.h`, or the relevant line in `firmware/tunerstudio/generated/rusefi_<board>.ini`), not just that the build/tests still pass — a stale generated file passes the same tests while silently not reflecting your source change.
...
- **`master` is reserved for the human — Claude never commits or pushes there without explicit per-action instruction.** The developer is not rusEFI's upstream maintainer; `master` is kept 1:1 with upstream rusEFI's `master` via the developer's own manual sync, so nothing should land on it except through that manual action. Leave `master`-targeted changes uncommitted in the working tree and summarize what changed instead.
- **Every other branch is free for Claude to `git commit` and `git push` on**, without asking for confirmation each time (standing directive, 2026-09-11: the developer uses non-`master` branches as their own workspace and has explicitly granted this authority). Still follow normal commit-message hygiene (see below) and the general "Executing actions with care" guidance for anything independently risky (force-push, history rewrite, deleting branches, etc. — those still need confirmation regardless of branch).
  - This supersedes an earlier, narrower exception that only named `master-imports-wip`/`master-imports-wip-sync` as personal WIP branches — those two are just ordinary non-`master` branches under this broader rule now.
  - **This section must survive upstream syncs.** After syncing/updating `master` from upstream, verify this section still states the broadened non-`master` commit authority (an upstream merge can revert or drop it) — if it's missing or reverted, restore it as part of the same unit of work.
