...
- **Never commit or push on `master` or any shared branch — only a human does either there.** Both `git commit` and `git push` are reserved for the human on `master` and any other shared/reviewed branch. Leave changes uncommitted in the working tree and summarize what changed; the human commits and pushes.
  - **Exception — personal WIP/sync branches** (currently `master-imports-wip` and `master-imports-wip-sync`): these are the developer's personal playground, not shared or reviewed. Claude may `git commit` and `git push` there when the developer asks, without a separate confirmation each time. This exception is scoped to those branches only — never extend it to `master` or any branch other than the ones named here without an explicit CLAUDE.md update.
